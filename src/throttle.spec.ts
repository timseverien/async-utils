import { throttleAll } from './throttle';
...
describe(throttleAll.name, () => {
	test.each<[number]>([
		...
		await expect(() =>
			throttleAll(promiseFactories, batchSize)
	...
	test('given Promise factories and batch size, returns the resolve values in order', async () => {
		...
		const result = await throttleAll(promiseFactories, batchSize);
	...
	test('given Promise factories and batch size, executes promises in a batched fashion', async () => {
		...
		await throttleAll(promiseFactories, batchSize);
